...
from com.wdbd.fd.model.dt_model import SERVER_STAUTS_OPEN, SERVER_STAUTS_CLOSING, ActionGroup
...
def start_dtserver() -> tl.Result:
    ...
    try:
        ...
        # 判断是否有服务正在运行
        count_of_runningserver = session.query(t_comm_server).filter(or_(t_comm_server.c.status == SERVER_STAUTS_OPEN, t_comm_server.c.status == SERVER_STAUTS_CLOSING)).count()
        if count_of_runningserver > 0:
            # 有服务正在运行，返回False
            return tl.Result(result=False, msg="当前有服务正在运行，无法新启动新服务")
        else:
            # 可以登记新服务
            session.execute(t_comm_server.insert().values({"status": SERVER_STAUTS_OPEN, "start_dt": tl.now(), "end_dt": ""}))
            session.commit()
...
def shundown_server() -> tl.Result:
    ...
    try:
        ...
        session.execute(t_comm_server.update().where(condition).values(updated_data))
...
def get_server_status() -> str:
    ...
    try:
        ...
        t_comm_server = DB_POOL.get("comm_server")
        res = session.query(t_comm_server.c.status).order_by(t_comm_server.c.start_dt.desc()).first()
        return res[0]
    except Exception as sqle:
        msg = "关闭新服务时遇到问题。" + str(sqle)
        tl.get_logger().error(msg)
...
def log_group(group: ActionGroup, result: bool = None, msg: str = None):
    ...
    try:
        ...
        rs_group = session.query(t_group).with_entities(t_group.c.id).filter(and_(t_group.c.group_name == group.name, t_group.c.status == ActionGroup.RUNNING))
        if rs_group.count() > 0:
            # 有正在运行的Group
            condition = (t_group.c.id == rs_group[0].id)
            if result:
                updated_data = {'status': ActionGroup.SUCCESS, "end_dt": tl.now(), "msg": msg}
            else:
                updated_data = {'status': ActionGroup.FAILED, "end_dt": tl.now(), "msg": msg}
            session.execute(t_group.update().where(condition).values(updated_data))
            session.commit()
        else:
            # 无记录，新增
            str_actions = ", ".join(group.actions)  # 拼接Action信息字符串
            session.execute(t_group.insert().values({"group_name": group.name, "status": ActionGroup.RUNNING, "start_dt": tl.now(), "end_dt": "", "msg": "", "actions": str_actions}))
            session.commit()
    except Exception as sqle:
