...
class CnStockDailyK:
    ...
    def download_all(self, start_date=None):
        ...
        try:
            ...
            with SQLiteDb() as db:
                # 取得全部的股票代码清单
                # count_stock = db.query_one("select count(code) as count_all from ods_baostock_stock_basic where type_id=1")['count_all']  
                sql = "select code from ods_baostock_stock_basic where type_id=1"  # FIXME 要修正
                rs = db.query(sql)
