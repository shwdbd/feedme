...
from com.wdbd.feedme.fd.ds_tushare.ts_stock import TsStockList, TsStockDaily
...
class App(tk.Tk):
    ...
    def set_window(self):
        ...
        win_width = 500
        win_height = 300
        self.geometry("{0}x{1}+{2}+{3}".format(win_width, win_height, int((self.winfo_screenwidth()-win_width)/2), int((self.winfo_screenheight()-win_height)/2)))     # 窗口大小
        self.resizable(False, False)

    # 创建主体部件
    def create_mainframe(self):
        # 日志内容框
        # self.text_log = ScrolledText(self, height=30, width=50)
        # self.text_log.grid(row=0, column=0, sticky=tk.NSEW, columnspan=1)
        ...
        tk.Button(self, text="刷新", command=self.reflash_status).grid(row=0, column=1, sticky=tk.N)

    def reflash_status(self):
        """ 初始化状态列表 """
        # TODO 待实现
        self.status_list.delete(0, tk.END)
        self.status_list.insert(tk.END, "最新数据日期:{0}".format(tl.d2viewstr(fd_api.get_last_date())))
        # TODO 最早数据
        # TODO 交易日历情况
    ...
    def create_menu(self):
        ...
        download_menu.add_command(label="下载今日数据", command=self.menu_download_today_onclick)
        download_menu.add_separator()
        # 历史数据下载
        download_history_data_menu = tk.Menu(download_menu, tearoff=False)
        download_history_data_menu.add_command(label="Tushare 股票日线数据", command=self.menu_about_onclick)
        download_menu.add_cascade(label="历史数据下载", menu=self.menu_download_today_onclick)    # TODO 连接command
        download_menu.add_command(label="下载交易日历", command=self.menu_download_today_onclick)    # TODO 连接command
...
class AboutWin(tk.Toplevel):
    ...
    def add_version(self):
        ...
        tk.Label(self, text="Feedme {ver}".format(ver=get_cfg("feedme", "version"))).pack()
        tk.Label(self, text="").pack()
        tk.Label(self, text="数据库配置文件地址：\n{path}".format(path=get_cfg("sqlite3", "db.path"))).pack()
...
class DownloadTodayWin(tk.Toplevel):
    ...
    def init_tk(self):
        ...
        tk.Label(self.topFrame, text="系统日期:").grid(row=0, column=0, sticky=tk.EW)
        tk.Label(self.topFrame, text="yyyy-MM-dd").grid(row=0, column=1, sticky=tk.W)
        tk.Label(self.topFrame, text="数据日期:").grid(row=1, column=0, sticky=tk.EW)
        # self.txt_date = tk.Entry(self, validate="focusout", validatecommand=self.validate_txt_date)
        self.txt_date = tk.Entry(self.topFrame)
        self.txt_date.grid(row=1, column=1, sticky=tk.EW)   # TODO 输入检验 validatecommand=(f, s1, s2, ...) va
        tk.Button(self.topFrame, text="下载", command=self.download).grid(row=1, column=3, sticky=tk.EW)
        self.topFrame.pack()
